...
def fetch_response():
    try:
                ...
                try:
                    ...
                    for sub_category in sub_categories:
                        ...
                        for sub_cat_item in sub_cat_items:
                            ...
                            while total > 0:
                                url_sub = (f'https://api.uk.misumi-ec.com/api/v1/series/search?lang=ENG&suppressResponseCode=true'
                                            f'&applicationId=72c49146-6b86-4155-a63c-499b00972294&_=1724920070194'
                                            f'&sessionId=undefined&field=%40search%2CseriesList.templateType'
                                            f'&categoryCode={item_id[4]}&sort=1&allSpecFlag=0&page={page}'
                                            f'&brandModeFlag=1&pageSize=45')

                                print(url_sub)
                                body = fetch_data(url_sub)
                                ...
                                for each_series_list in seriesList:
                                    try:
                                        ...
                                        data_row = [category, item_id[3], sub_cat_label, item_name, item_id[4],
                                                    departmentcode, minStandardDaysToShip, maxStandardDaysToShip,
                                                    seriesCode, seriesName, brandCode, brandName]
                                        writer.writerow(data_row)
                                    except KeyError as e:
                                        print(f"Key error: {e}")

                                total = len(seriesList)  # Adjust logic for total count if needed
                                page += 1
                except KeyError:
                    continue

        # Convert list to DataFrame and save to CSV and Excel
        ...
        print("Data saved to 'categories_subcategories_item.csv'")
        df.to_excel('categories_subcategories_item.xlsx', index=False)
        print("Data saved to 'categories_subcategories_item.xlsx'")
